...
class FpsListenserImpl(IFpsListener):
    ...
    def report_fps_info(self, fps_info, devices):
        ...
        source_mobileperf_folder = os.path.join(base_path)  # 源MobilePerf文件夹路径
        target_device_id = devices.replace(':', '_').replace('.', '_')
        # file_path = os.path.join(base_path, "R", f"_{target_device_id}", "results", f"{str(fps_info.pkg_name)}", "fps_data.csv")
