...
class FpsCollector(object):
    ...
    #
    #     '''
    #     cmd = "adb -s " + self.device + " shell dumpsys activity | grep topResumedActivity"
    #     # print(cmd)
    ...
    def get_focus_window(self):
        ...
        cmd = f"adb -s {self.device} shell dumpsys activity activities | grep \"mCurrentFocus\""
        window_info_raw = os.popen(cmd).readlines()

        # 如果列表为空，说明没有获取到焦点窗口信息
        if not window_info_raw:
            self.package_name = None
            self.focus_window = None
            return

        window_info = str(window_info_raw[0])

        # 使用正则表达式提取包名和窗口名
        match = re.match(r'.* u\d+ (\S+)/(\S+)}.*', window_info)
        if match:
            package_name = match.group(1)
            window_name = match.group(1) + "/" + match.group(2)
        else:
            package_name = None
            window_name = None
    ...
    def _calculate_results(self, timestamps):
        ...
        frame_count = len(timestamps)
        jank_list, caton, vsyncOverTimes = self._calculate_janky(timestamps)
        # 计算 FPS 时需要考虑是否有 Vsync 超时
        total_frames = frame_count + vsyncOverTimes
        fps = frame_count / total_frames * 60 if total_frames != 0 else 0
        return fps, jank_list, caton
    ...
    def _get_fps_data(self):
        ...
        for line in results.readlines():
            ...
            if isHaveFoundWindow and PROFILEDATA_line == 1:
                ...
                fields = line.split(",")
                #print(fields)
                each_frame_timestamp = [float(fields[2]), float(fields[19])]
